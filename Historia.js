...
function filtrujTransakcje() {
    ...
    if (selectedDate) {
        ...
        filteredTransakcje = filteredTransakcje.filter(transakcja => {
            const transakcjaDate = new Date(transakcja[3]);
            switch (operator) {
                case '>': return transakcjaDate > filterDate;
                case '<': return transakcjaDate < filterDate;
                case '=': return transakcjaDate.toDateString() === filterDate.toDateString();
                default: return true;
            }
        });
    }
    ...
    if (filteredTransakcje.length > 0) {
        ...
        listaTransakcji.innerHTML = `<div class="brak-transakcji" style="color: gray; font-style: italic; text-align: center;">Brak wyników dla wybranych filtrów</div>`;
    }
}
